  ...
  <div class="feedback-tab">
    ...
    <div v-else class="feedback-form">
      ...
      <div class="form-group">
        ...
        <textarea v-model="feedback.content" :placeholder="getText('contentPlaceholder')" maxlength="1000"></textarea>
      ...
      <div class="form-group">
        ...
        <div class="category-pills">
            <button v-for="cat in categories" :key="cat" @click="feedback.category = cat" :class="{ active: feedback.category === cat }">
...
export default {
  ...
  computed: {
    texts() {
      const allTexts = {
        ko: {
          title: '피드백 보내기', subtitle: '여러분의 소중한 의견으로 Devvy Bot이 발전합니다.',
          ratingLabel: '만족도', contentLabel: '의견', categoryLabel: '분야',
          contentPlaceholder: '개선점, 칭찬, 버그 리포트 등 무엇이든 좋습니다.',
          submit: '제출하기', submitting: '제출 중...', thankYou: '피드백 감사합니다!',
          sendMore: '피드백 더 보내기',
          categories: { ui_ux: 'UI/UX', performance: '성능', functionality: '기능 정확도', other: '기타' }
        },
        en: {
          title: 'Send Feedback', subtitle: 'Your feedback helps Devvy Bot improve.',
          ratingLabel: 'Satisfaction', contentLabel: 'Comment', categoryLabel: 'Category',
          contentPlaceholder: 'Any suggestions, praise, or bug reports are welcome.',
          submit: 'Submit', submitting: 'Submitting...', thankYou: 'Thank you for your feedback!',
          sendMore: 'Send More Feedback',
          categories: { ui_ux: 'UI/UX', performance: 'Performance', functionality: 'Accuracy', other: 'Other' }
        }
      };
      return allTexts[this.currentLanguage];
    },
    canSubmit() {
      return this.feedback.rating > 0 && this.feedback.content.trim().length > 5 && !this.isSubmitting;
    }
  },
  methods: {
    getText(key) { return this.texts[key] || key; },
    resetForm() {
      this.feedback = { rating: 0, content: '', category: '' };
      this.successMessage = '';
      this.errorMessage = '';
