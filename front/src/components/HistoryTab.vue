...
export default {
  ...
  computed: {
    texts() {
      const allTexts = {
        ko: {
          title: '대화 히스토리', refresh: '새로고침', retry: '재시도', messages: '개 메시지',
          noHistory: '대화 기록이 없습니다.', startChatting: 'Devvy Bot과 새로운 대화를 시작해보세요.',
          startChat: '채팅 시작하기', today: '오늘', yesterday: '어제'
        },
        en: {
          title: 'Chat History', refresh: 'Refresh', retry: 'Retry', messages: 'messages',
          noHistory: 'No chat history found.', startChatting: 'Start a new conversation with Devvy Bot.',
          startChat: 'Start Chat', today: 'Today', yesterday: 'Yesterday'
        }
      };
  ...
  methods: {
    ...
    viewDetail(item) { this.$emit('view-detail', item); },
    ...
    formatTime(dateStr) {
        const date = new Date(dateStr);
        return date.toLocaleTimeString(this.currentLanguage === 'ko' ? 'ko-KR' : 'en-US', { hour: '2-digit', minute: '2-digit' });
    }
  },
  watch: {
    loading(newVal) {
